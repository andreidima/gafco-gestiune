        ...
        <form class="resource-filter-panel">
            <div class="row g-2 align-items-end">
                <div class="col-md-7 col-xl-5">
                    ...
                    <input id="dispatch-search" name="search" value="{{ request('search') }}" class="form-control" placeholder="Numar sau titlu">
                </div>
                ...
                <table class="table resource-table">
                    ...
                    @forelse($unassignedTasks as $task)
                        <tr>
                            ...
                            <td><div class="resource-cell-stack"><span class="resource-primary">{{ $task->sourceLocation?->code ?? '-' }} <span aria-hidden="true">&rarr;</span> {{ $task->destinationLocation?->code ?? '-' }}</span>@if($task->sourceLocation || $task->destinationLocation)<span class="resource-secondary">{{ $task->sourceLocation?->name ?? 'Sursa nespecificata' }} / {{ $task->destinationLocation?->name ?? 'Destinatie nespecificata' }}</span>@endif</div></td>
                                ...
                                <form method="post" action="{{ route('tasks.assignments.store', $task) }}" class="dispatch-assignment-form">
                                    ...
                                    <select name="driver_id" class="form-select form-select-sm" required aria-label="Alege sofer pentru {{ $task->number }}">
                                        ...
                                        @foreach($driverSummaries as $summary)
                                            <option value="{{ $summary['driver']->id }}">{{ $summary['driver']->name }} - {{ $summary['stateLabel'] }}</option>
                                        @endforeach
                                    </select>
