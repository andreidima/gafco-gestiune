    ...
    <form method="post" action="{{ $editing ? route('users.update', $user) : route('users.store') }}" class="resource-form-card">
        ...
        <div class="resource-form-section">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4"><label class="form-label">Roluri</label><select name="roles[]" class="form-select" multiple data-tom-select>@foreach($roles as $role)<option value="{{ $role->name }}" @selected(in_array($role->name, old('roles', $user?->roles?->pluck('name')->all() ?? [])))>{{ $role->name }}</option>@endforeach</select></div>
